...
def get_data(address,year,quater,page):
    ...
    page_num = page
    # try:
    response = requests.get(url,params={'s_i':'new_dlhy','reportdate':year,'quarter':quater,'p':page_num})
    response.raise_for_status()
    response.encoding = response.apparent_encoding
    html = response.text
    soup = BeautifulSoup(html, 'html.parser')
...
db_name = "cc"
fi_url_sina = {}
fi_url_sina['profit'] = 'http://vip.stock.finance.sina.com.cn/q/go.php/vFinanceAnalyze/kind/profit/index.phtml/num=6000'
